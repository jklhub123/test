...
void Set_all_pressure2(int pressure)
{
	...
	data[5] = 0;
	data[6] = 0;
	data[7] = 0; // 气压数组初始化：[0-1]p0,[2-3]p1,[4-5]p2,[6-7]p3
	joint0.P = pressure;
	joint0.Q = pressure;
	joint1.P = pressure;
	joint1.Q = pressure;
...
void MOV_mf_1ring(int hz)
{
	...
	for (int i = 0; i <= max; i++)
	{
		...
		while (1)
		{
			...
			if ((xTaskGetTickCount() - startTick) >= pdMS_TO_TICKS(200))
			{
				break;
			}
		}
	...
	while (1) // 保持在终点
	{
		...
		if ((xTaskGetTickCount() - remain_Tick) >= pdMS_TO_TICKS(4000))
		{
			break;
		}
	}
...
// 			return;
// 		}
// 		if ((xTaskGetTickCount() - remain_Tick_1) >= pdMS_TO_TICKS(5000))
// 		{
// 			break;
// 		}
// 	}
