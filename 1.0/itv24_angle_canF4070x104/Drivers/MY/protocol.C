  ...
  while(1)
  {
    ...
    switch (cmd_type)
    {
     ...
     case SET_P_I_D_CMD:
     {
       ...
       temp_p.i = COMPOUND_32BIT(&frame_data[9]);
       temp_i.i = COMPOUND_32BIT(&frame_data[13]);
       temp_d.i = COMPOUND_32BIT(&frame_data[17]);
       
       if (packet.ch == CURVES_CH0)
       {
         set_p_i_d(&pidcontrol0, temp_p.f, temp_i.f, temp_d.f);    //设置 P I D
         set_p_i_d(&pidcontrol1, temp_p.f, temp_i.f, temp_d.f);    //设置 P I D
       }
    //    else if (packet.ch == CURVES_CH1)
    //    {
    //      set_p_i_d(&pidcontrol1, temp_p.f, temp_i.f, temp_d.f);    //设置 P I D
    //    }
        else if (packet.ch == CURVES_CH2)
       {
         set_p_i_d(&pidcontrol2, temp_p.f, temp_i.f, temp_d.f);    //设置 P I D
         set_p_i_d(&pidcontrol3, temp_p.f, temp_i.f, temp_d.f);    //设置 P I D
       }
    ...
    //    }
       ...
       {
         set_p_i_d(&pidcontrol4, temp_p.f, temp_i.f, temp_d.f);    //设置 P I D
         set_p_i_d(&pidcontrol5, temp_p.f, temp_i.f, temp_d.f);
       }
    //    else if (packet.ch == CURVES_CH5)
    //    {
    //      set_p_i_d(&pidcontrol5, temp_p.f, temp_i.f, temp_d.f);    //设置 P I D
    //    }
        else if (packet.ch == CURVES_CH6)
       {
         set_p_i_d(&pidcontrol6, temp_p.f, temp_i.f, temp_d.f);    //设置 P I D
         set_p_i_d(&pidcontrol7, temp_p.f, temp_i.f, temp_d.f);
       }
    //     else if (packet.ch == CURVES_CH7)
    //    {
    //      set_p_i_d(&pidcontrol7, temp_p.f, temp_i.f, temp_d.f);    //设置 P I D
    //    }
     }
     break;
...
/*
     case SET_TARGET_CMD:
     {	
				type_cast_t target_temp;
       target_temp.i=COMPOUND_32BIT(&frame_data[9]);    // 设定目标值
       packet.ch = frame_data[CHX_INDEX_VAL];
//        int temp=(int)(target_temp.f*8192/180);
				int temp=target_temp.f*8192/180;
//				HAL_UART_Transmit(&huart1, frame_data, 10, 0xFFFFF);
//				printf("=%d",temp);
//printf("asdad=%d",packet.ch);
				
       if (packet.ch == CURVES_CH0)    
       {
         pidcontrol0.SetVal_real=temp;
         set_pid_target(0);					
       }
       else if (packet.ch == CURVES_CH1)    
       {
         pidcontrol1.SetVal_real=temp;
					set_pid_target(0);
       }
       else if (packet.ch == CURVES_CH2)    
       {
         pidcontrol2.SetVal_real=temp;
					set_pid_target(1);
       }
				else if (packet.ch == CURVES_CH3)    
       {
         pidcontrol3.SetVal_real=temp;
					set_pid_target(1);
       }
				else if (packet.ch == CURVES_CH4)    
       {
         pidcontrol4.SetVal_real=temp;
//					printf("====%d",pidcontrol4.SetVal_real);
					set_pid_target(2);
       }
       else if (packet.ch == CURVES_CH5)    
       {
         pidcontrol5.SetVal_real=temp;
					set_pid_target(2);
       }
       else if (packet.ch == CURVES_CH6)    
       {
         pidcontrol6.SetVal_real=temp;
					set_pid_target(3);
       }
				else if (packet.ch == CURVES_CH7)    
       {
         pidcontrol7.SetVal_real=temp;
					set_pid_target(3);                  // 解耦
				}           
...
//      
      case STOP_CMD:
      {
        outzero=1;
		pidstart=0;
		PID_Reset(&pidcontrol4);
		PID_Reset(&pidcontrol5);
		PID_Reset(&pidcontrol6);
		PID_Reset(&pidcontrol7);
      }
      break;
      ...
      /*
                case SET_AIRWAY_CMD:
     {
                pidcontrol4.airwayP = COMPOUND_16BIT(&frame_data[7]);     //PN  可能反了
				pidcontrol4.airwayN = COMPOUND_16BIT(&frame_data[9]); 
				pidcontrol5.airwayP = COMPOUND_16BIT(&frame_data[11]);
				pidcontrol5.airwayN = COMPOUND_16BIT(&frame_data[13]);
				pidcontrol6.airwayP = COMPOUND_16BIT(&frame_data[15]);
				pidcontrol6.airwayN = COMPOUND_16BIT(&frame_data[17]);
				pidcontrol7.airwayP = COMPOUND_16BIT(&frame_data[19]);
				pidcontrol7.airwayN = COMPOUND_16BIT(&frame_data[21]);
     }
     break;
      */
      ...
      { 
				...
				for(i=0;i<4;i++){
				setchannel[i] = 10 * (frame_data[8*i+7] - '0')	//calculate ctrl channel
									+ (frame_data[8*i+8] - '0');

				setvalue[i] = 1000 * (frame_data[8*i+10] - '0')	//calculate ctrl value
									+ 100 * (frame_data[8*i+11] - '0')
									+ 10 * (frame_data[8*i+12] - '0')
									+ 1 * (frame_data[8*i+13] - '0');
//										set_computer_value(SEND_FACT_CMD, 1, value, 4);
				}
				outstart=1;
//OutPut(setchannel[0],setvalue[0],1);
...
void send_computer_pressure_value(uint8_t cmd, uint8_t ch, void *pressure, uint8_t num)//2023-11-20
{
//    uint8_t sum = 0;    // 校验和
    num *= 2;           // 一个参数 2 个字节
    ...
    set_packet.cmd  = cmd;             		// 设置命令
    HAL_UART_Transmit(&huart2, (uint8_t *)&set_packet, sizeof(set_packet), 0xFFFFF);    // 发送数据头
    HAL_UART_Transmit(&huart2, (uint8_t *)pressure, num, 0xFFFFF);                      // 发送参数  	
}
...
{
	pid->Kp_angle = p;    // éè??±èày?μêy P
	pid->Ki_angle = i;    // éè???y·??μêy I
	pid->Kd_angle = d;    // éè???￠·??μêy D
}
...
//	
//    if (armnum == 0){	
//		Angel0=(float)pidcontrol0.SetVal_real / 8192 * 3.141;
//		Angel1=(float)pidcontrol1.SetVal_real / 8192 * 3.141;
//		fAngel0=asinf(1.1412/2*(sinf(Angel0)+cosf(Angel0)*sinf(Angel1)));
//		fAngel1=atanf(1.1412/2*(tanf(Angel1)-tanf(Angel0)/cosf(Angel1)));
//		pidcontrol0.SetVal=(int)(fAngel0/3.14159*8192);
//		pidcontrol1.SetVal=(int)(fAngel1/3.14159*8192);
//		}
//	else if (armnum == 1){	
//		Angel0=(float)pidcontrol2.SetVal_real / 8192 * 3.141;
//		Angel1=(float)pidcontrol3.SetVal_real / 8192 * 3.141;
//		fAngel0=asinf(1.1412/2*(sinf(Angel0)+cosf(Angel0)*sinf(Angel1)));
//		fAngel1=atanf(1.1412/2*(tanf(Angel1)-tanf(Angel0)/cosf(Angel1)));
//		pidcontrol2.SetVal=(int)(fAngel0/3.14159*8192);
//		pidcontrol3.SetVal=(int)(fAngel1/3.14159*8192);
//		}
//	else if (armnum == 2){	
//		Angel0=(float)pidcontrol4.SetVal_real / 8192 * 3.141;
//		Angel1=(float)pidcontrol5.SetVal_real / 8192 * 3.141;
//		fAngel0=asinf(1.1412/2*(sinf(Angel0)+cosf(Angel0)*sinf(Angel1)));
//		fAngel1=atanf(1.1412/2*(tanf(Angel1)-tanf(Angel0)/cosf(Angel1)));
//		pidcontrol4.SetVal=(int)(fAngel0/3.14159*8192);
//		pidcontrol5.SetVal=(int)(fAngel1/3.14159*8192);
////		  printf("aaaaaaa=%d",pidcontrol4.SetVal);
//		}
//	else if (armnum == 3){	
//		Angel0=(float)pidcontrol6.SetVal_real / 8192 * 3.141;
//		Angel1=(float)pidcontrol7.SetVal_real / 8192 * 3.141;
//		fAngel0=asinf(1.1412/2*(sinf(Angel0)+cosf(Angel0)*sinf(Angel1)));
//		fAngel1=atanf(1.1412/2*(tanf(Angel1)-tanf(Angel0)/cosf(Angel1)));
//		pidcontrol6.SetVal=(int)(fAngel0/3.14159*8192);
//		pidcontrol7.SetVal=(int)(fAngel1/3.14159*8192);
//		}
//}
